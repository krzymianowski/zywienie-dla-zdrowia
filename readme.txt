...
The parameter-free `[zywienie_dla_zdrowia]` shortcode directly composes the configured `[zfdz_jadlospisy]` renderer followed by the configured `[zfdz_badania]` renderer. Each enabled renderer runs exactly once. The aggregate adds only static wrapper HTML and does not inspect catalogs, select results, build URLs, change child messages, add a cache, or include `[zfdz_jadlospisy_archiwum]`. Child shortcodes remain independently available, and the aggregate adds no attributes, CSS, JavaScript, or archive-result shortcode.

The **Żywienie dla Zdrowia → Ustawienia** page uses the native Settings API and `manage_options` to save exactly one array option, `zfdz_settings_v1`. Its boolean keys and defaults are: `overview_show_menus=true`, `overview_show_lab_results=true`, `menu_show_upcoming=true`, and `open_documents_new_tab=false`. A missing option is never written during reads and preserves the previous public behavior; partial arrays inherit missing defaults and unknown submitted keys are discarded. Aggregate visibility never disables standalone shortcodes. Hiding upcoming menus changes only `[zfdz_jadlospisy]` presentation, not the full catalog, archive, menu fingerprint, or laboratory matching. New-tab mode adds exactly `target="_blank" rel="noopener noreferrer"` to public PDF links without changing their URLs.

Saving is handled by `options.php` with the standard Settings API nonce and no custom endpoint. The page also lists all four public shortcodes. No setting changes managed directory names, public messages, matching/latest/fallback rules, publication decisions, cache keys, or the fixed 300-second TTLs. There is no settings transient, REST exposure, CSS/JavaScript, editable message, protected download endpoint, or uninstall cleanup in this stage.
...
This development version provides technical administration status and settings pages, public current/upcoming and archive menu shortcodes, the public candidate-only laboratory-result shortcode, and a thin configurable aggregate shortcode for the current menu and laboratory-result views. For development, place the repository in the WordPress plugins directory and install development dependencies with Composer. Do not use it as a substitute for a production-ready release.

== Frequently Asked Questions ==

= Does this version display documents or provide shortcodes? =

The parameter-free `[zfdz_jadlospisy]` shortcode displays grouped current and upcoming validated menu candidates. The separate parameter-free `[zfdz_jadlospisy_archiwum]` shortcode displays grouped archived periods newest first. `[zfdz_badania]` displays only a latest matched technical candidate and otherwise a safe state-specific message. `[zywienie_dla_zdrowia]` composes the current/upcoming menu output followed by the laboratory-result output without including the archive. Remaining module shortcodes are still planned.
...
* Added native WordPress Settings API configuration under **Żywienie dla Zdrowia → Ustawienia**, stored as the single `zfdz_settings_v1` option with backward-compatible public-view defaults.
* Added the parameter-free `[zywienie_dla_zdrowia]` aggregate shortcode composing the existing current/upcoming menu and laboratory-result renderers in that order.
